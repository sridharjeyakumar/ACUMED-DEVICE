    ...
    const handleEditSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const formattedData: any = {
                ...
                material_type: formData.material_type,
                material_spec: formData.material_spec || null,
                safety_stock_qty: formData.safety_stock_qty ? Number(formData.safety_stock_qty) : null,
                re_order_qty: formData.re_order_qty ? Number(formData.re_order_qty) : null,
                min_order_qty: formData.min_order_qty ? Number(formData.min_order_qty) : null,
                lead_time_days_min: formData.lead_time_days_min ? Number(formData.lead_time_days_min) : null,
                lead_time_days_max: formData.lead_time_days_max ? Number(formData.lead_time_days_max) : null,
                shelf_life_in_months: formData.shelf_life_in_months ? Number(formData.shelf_life_in_months) : null,
                gr_tolerance_percent: formData.gr_tolerance_percent ? Number(formData.gr_tolerance_percent) : null,
                qc_required: formData.qc_required,
                coa_checklist_id: formData.qc_required ? (formData.coa_checklist_id || null) : null,
                material_image: formData.material_image || null,
                material_image_icon: formData.material_image_icon || null,
                active: formData.active,
                last_modified_user_id: "ADMIN",
...
const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
   ...
   const confirmCancelItem = async () => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            await materialAPI.delete(materialToDelete.material_id);
            setMaterials(prev => prev.filter(m => m.material_id !== materialToDelete.material_id));
            toast({
                title: "Deleted",
                description: `Material ${materialToDelete.material_name} has been deleted.`,
            });
                        ...
                        >
                            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
                                <div className="flex items-center justify-between p-6 bg-red-600 rounded-t-xl">
                                    <h2 className="text-xl font-bold text-white">Delete Material</h2>
                                    <button
                                        onClick={() => setIsDeleteDialogOpen(false)}
                                        className="text-white hover:opacity-80 rounded-lg p-2 transition-colors"
                                    >
                                ...
                                <div className="p-6 space-y-4">
                                    <p className="text-gray-700">
                                        Are you sure you want to permanently delete <span className="font-semibold">{materialToDelete?.material_name}</span>? This action cannot be undone.
                                    </p>
                                    <div className="flex items-center justify-end gap-4">
                                        <Button
                                            variant="outline"
