    ...
    const handleDetailRowChange = (index: number, e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        setDetailRows(prev => prev.map((row, i) => {
            ...
            if (name === 'expectedResult') {
                if (!['R', 'E', 'G', 'L'].includes(value)) updated.expectedValue1 = '';
                if (value !== 'R') updated.expectedValue2 = '';
                if (value !== 'T') updated.expectedText = '';
            }
            return updated;
        }));
    };
        ...
        try {
            ...
            for (const [index, row] of detailRows.entries()) {
                if (row.checklistParameter) {
                    await coaChecklistDetailAPI.create(detailToSnakeCase(formData.checklistId, { ...row, checklistSno: String(index + 1) }));
                }
            }
            ...
            for (const [index, row] of detailRows.entries()) {
                ...
                if (row._id) {
                    await coaChecklistDetailAPI.update(
                        ...
                        safeInteger(row.checklistSno) || sno,
                        detailToSnakeCase(selectedChecklist.checklistId, { ...row, checklistSno: String(sno) })
                    );
                } else {
                    await coaChecklistDetailAPI.create(detailToSnakeCase(selectedChecklist.checklistId, { ...row, checklistSno: String(sno) }));
                }
            }
                                            ...
                                            <table className="w-full text-sm">
                                                <thead>
                                                    <tr className="bg-slate-50 border-b border-slate-200">
                                                        <th className="px-3 py-2 text-center font-semibold text-slate-600 whitespace-nowrap w-12">S.no</th>
                                                        <th className="px-3 py-2 text-left font-semibold text-slate-600 whitespace-nowrap">Parameter <span className="text-red-500">*</span></th>
                                                        <th className="px-3 py-2 text-left font-semibold text-slate-600 whitespace-nowrap">Expected Result <span className="text-red-500">*</span></th>
                                                        <th className="px-3 py-2 text-left font-semibold text-slate-600 whitespace-nowrap">Expected Text</th>
                                                        <th className="px-3 py-2 text-left font-semibold text-slate-600 whitespace-nowrap">Value 1</th>
                                                        <th className="px-3 py-2 text-left font-semibold text-slate-600 whitespace-nowrap">Value 2</th>
                                                    ...
                                                    {detailRows.map((row, index) => (
                                                        <tr key={index} className="hover:bg-slate-50/50">
                                                            ...
                                                            <td className="px-2 py-2">
                                                                <Input name="checklistParameter" value={row.checklistParameter} onChange={e => handleDetailRowChange(index, e)} placeholder="e.g., Perforation" required maxLength={50} />
                                                            </td>
                                                            <td className="px-2 py-2">
                                                                <select name="expectedResult" value={row.expectedResult} onChange={e => handleDetailRowChange(index, e)} required className="w-full border border-input rounded-md px-2 py-2 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-ring">
                                                            ...
                                                            <td className="px-2 py-2">
                                                                <Input
                                                                    name="expectedText"
                                                                    value={row.expectedText}
                                                                    onChange={e => handleDetailRowChange(index, e)}
                                                                    placeholder={row.expectedResult === 'T' ? "e.g., NLT 98.0%" : "—"}
                                                                    maxLength={25}
                                                                    required={row.expectedResult === 'T'}
                                                                    disabled={row.expectedResult !== 'T'}
                                                                    className={row.expectedResult !== 'T' ? "bg-slate-100 text-slate-400 cursor-not-allowed" : ""}
                                                                />
                                                            </td>
                                                            <td className="px-2 py-2">
                                                                <Input
                                                                    ...
                                                                    step="0.001"
                                                                    name="expectedValue1"
                                                                    value={row.expectedValue1}
                                                                    onChange={e => handleDetailRowChange(index, e)}
                                                                    placeholder={['R','E','G','L'].includes(row.expectedResult) ? "0.000" : "—"}
                                                                    required={['R','E','G','L'].includes(row.expectedResult)}
                                                                    disabled={!['R','E','G','L'].includes(row.expectedResult)}
                                                                    className={`w-24 ${!['R','E','G','L'].includes(row.expectedResult) ? "bg-slate-100 text-slate-400 cursor-not-allowed" : ""}`}
                                                                />
                                                            </td>
                                                            <td className="px-2 py-2">
                                                                <Input
                                                                    ...
                                                                    step="0.001"
                                                                    name="expectedValue2"
                                                                    value={row.expectedValue2}
                                                                    onChange={e => handleDetailRowChange(index, e)}
                                                                    placeholder={row.expectedResult === 'R' ? "0.000" : "—"}
                                                                    required={row.expectedResult === 'R'}
                                                                    disabled={row.expectedResult !== 'R'}
                                                                    className={`w-24 ${row.expectedResult !== 'R' ? "bg-slate-100 text-slate-400 cursor-not-allowed" : ""}`}
                                                                />
                                                            </td>
                                                            <td className="px-2 py-2 text-center">
                                                                {detailRows.length > 1 && (
                                                                    <button type="button" onClick={() => removeDetailRow(index)} className="text-red-400 hover:text-red-600 p-1 rounded hover:bg-red-50">
                                                                        <X className="w-4 h-4" />
                                                                    </button>
                                                                )}
                                                            </td>
